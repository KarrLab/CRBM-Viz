...
def test_extract_hdf5_metadata_with_nan():
  ...
  data_list_mixed: list[float | str] = [str(x) if np.isnan(x) or np.isinf(x) else x for x in data_list_float] # type: ignore
  ...
  dataset_data_float = DatasetData.model_validate_json(json.dumps(dict(shape=(1075,), values=data_list_float)), strict=True)
  dataset_data_mixed = DatasetData.model_validate_json(json.dumps(dict(shape=(1075,), values=data_list_mixed)), strict=True)
